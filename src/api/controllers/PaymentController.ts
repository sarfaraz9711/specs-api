...
export class PaymentController {
     ...
     * @apiParam (Request body) {Number} paymentMethod search by paymentMethod
    ...
    public async paymentList(@QueryParam('limit') limit: number, @QueryParam('offset') offset: number, @QueryParam('paymentMethod') paymentMethod: number,
                             @QueryParam('startDate') startDate: string, @QueryParam('endDate') endDate: string, @QueryParam('count') count: number | boolean, @QueryParam('email') email: string,@QueryParam('orderId') orderId: string,@QueryParam('customerName') customerName: string,@QueryParam('mobile') mobile: string,@QueryParam('fromDate') fromDate: string,@QueryParam('toDate') toDate: string, @Res() response: any): Promise<any> {

        
        let  startDateMin:any
        if (startDate && startDate !== '') {
         startDateMin = moment(startDate).subtract(5, 'hours').subtract(30, 'minutes').format('YYYY-MM-DD HH:mm:ss');
        }
        const date = endDate + ' 23:59:59';
        
        let endDateMin:any
        if (endDate && endDate !== '') {
         endDateMin = moment(date).subtract(5, 'hours').subtract(30, 'minutes').format('YYYY-MM-DD HH:mm:ss');
        }
        const select = [
        ...
        //Previous code payload customer name searching orderId
    ...
    @Authorized(['admin', 'archive-payment-list-export-excel'])
