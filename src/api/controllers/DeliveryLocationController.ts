...
export class DeliveryLocationController {
    ...
    @Authorized(['admin', 'add-delivery-location'])
    public async addDeliveryLocation(@Body({ validate: true }) deliveryLocationParam: CreateDeliveryLocationRequest, @Req() request: any, @Res() response: any): Promise<any> {
        ...
        if (existDelivery) {
            const errorResponse: any = {
                status: 0,
                message: 'You have already added this zipcode.',
            };
        ...
        if (createDeliveryLocation) {
            const successResponse: any = {
                status: 1,
                message: 'Successfully created delivery location.',
    ...
    @Authorized(['admin', 'delivery-location-delete'])
    public async deleteDeliveryLocation(@Param('deliveryLocationId') deliveryLocationId: number, @Req() request: any, @Res() response: any): Promise<any> {
        ...
        if (deleteDeliveryLocation) {
            ...
            const errorResponse: any = {
                status: 0,
                message: 'Unable to delete the Delivery Location.',
            };
            return response.status(400).send(errorResponse);
        }
    }
    ...
    @Authorized(['admin', 'update-delivery-location'])
    public async updateDeliveryLocation(@Body({ validate: true }) updateParam: CreateDeliveryLocationRequest,
        ...
        const existDelivery = await this.deliveryLocationService.findOne({ where: {zipCode: updateParam.zipCode, deliveryLocationId: Not(deliveryLocation.deliveryLocationId), vendorId: 0}});
        if (existDelivery) {
            const errorResponse: any = {
                status: 0,
                message: 'You have already added this zipcode.',
    ...
    @Authorized(['admin', 'delivery-location-list'])
