...
import { Get, Post, Put, Delete, Body, JsonController, Authorized, QueryParam, Res, Req, Param } from 'routing-controllers';
...
export class CategoryController {
    ...
    public async addCategory(@Body({ validate: true }) category: AddCategory, @Res() response: any): Promise<Category> {
        ...
        _setting.createQueryBuilder().update().set({categoryFlag:currentDate}).execute()
        ...
        newCategory.metaTagTitle = category.metaTagTitle ? category.metaTagTitle : category.name;
    ...
    public async updateCategory(@Body({ validate: true }) category: UpdateCategoryRequest, @Res() response: any, @Req() request: any): Promise<Category> {
        ...
        // categoryId.metaTagTitle = category.metaTagTitle ? category.metaTagTitle : category.name;
        // const metaTagTitle = category.categorySlug ? category.categorySlug : category.name;
        // const data = metaTagTitle.replace(/\s+/g, '-').replace(/[&\/\\@#,+()$~%.'":*?<>{}]/g, '').toLowerCase();
        // categoryId.categorySlug = await this.validate_slug(data);
        categoryId.categorySlug = category.categorySlug
     ...
     */
    @Get('/categorylist')
    @Authorized(['admin', 'list-category'])
    ...
    /**
     * @api {get} /api/category-export-all Caetgory Export All API
     * @apiGroup Category
     * @apiParam (Request body) {Number} status status
     * @apiParam (Request body) {String} keyword keyword
     * @apiParam (Request body) {Number} sortOrder sortOrder
     ...
     public async categoryExportAll(@QueryParam('status') status: number, @QueryParam('keyword') keyword: string,  @QueryParam('sortOrder') sortOrder: number, @Req() request: any, @Res() response: any): Promise<any> {
        ...
        if (sortOrder) {
            ...
            sort.push({
             ...
             // Excel sheet column define
             worksheet.columns = [
                {header: 'CategoryId', key :'categoryId', size: 16, width: 30 },
                { header: 'Image', key: 'image', size: 16, width: 30 },
                { header: 'Category Name', key: 'name', size: 16, width: 30 },
                { header: 'Levels', key: 'parentInt', size: 16, width: 60 },
                { header: 'Sort Order', key: 'sortOrder', size: 16, width: 15 },
            ...
            for (const data of categoryList) {
                ...
                rows.push([data.categoryId, data.image, data.name, data.levels, data.sortOrder, data.isActive]);
            }
         // Add all rows data in sheet
         worksheet.addRows(rows);
    ...
    @Get('/get-parent-category-by-category-id/:categoryId')
    public async getParentCategoryByCategoryId(@Param('categoryId') categoryId:any) {
        const _category = getManager().getRepository(Category)
        const result = await _category.createQueryBuilder('c')
        .select(['ct.*'])
        .leftJoin(Category, 'ct', 'c.parent_int=ct.category_id')
        .where('c.category_id=:categoryId',{categoryId:categoryId})
        .getRawOne()
