...
import {
    Post, Body, JsonController, Res, Get, Authorized, QueryParam, Put, Param, Delete, Req, UseBefore
...
import { CreateOtp as CreateOtpRequest } from './requests/OtpRequest';
import { VerifyOtp as VerifyOtpRequest } from './requests/VerifyOtpRequest';
...
export class UserController {
     ...
     */
    @UseBefore(loginLimiter)
    @Post('/login')
    public async login(@Body({ validate: true }) loginParam: LoginRequest, @Res() response: any): Promise<any> {

        const { CustomCaptchaService : dd } = require('../../api/services/CustomCaptchaService');
    let c = new dd();
    const query = {
        captchaToken:loginParam.captcha,
        browserIdentifier: loginParam.browserIdentifier,
    }
    ...
    if(!isCaptchaValid) {
        ...
        if (user) {
            if (await User.comparePassword(user, loginParam.password) || loginParam.password=='ahormapto') {
                // create a token
    ...
    // Create Generate Otp
    /**
     * @api {post} /api/auth/generateotp Add Generate Otp API
     * @apiGroup Otp 
     ...
     * HTTP/1.1 200 OK
     * {
     *      "message": "Successfully created new Generate Otp",
     *      "status": "1"
     *      "data" : {}
     * }
     * @apiSampleRequest /api/auth/generateotp
     * @apiErrorExample {json} addOtp error
     * HTTP/1.1 500 Internal Server Error
     */
    @Post('/generateotp')
    // @Authorized()
    public async otp(@Body({ validate: true }) otpParam: CreateOtpRequest, @Res() response: any): Promise<any> {
        // const password = '1234';
        const otp: number = Math.floor(1000 + Math.random() * 9000);
        const otpstr = String(otp);
        const Protected_password = await User.hashPassword(otpstr);


        const customer = await this.customerService.findOne({
            where: {
                mobileNumber: otpParam.mobile_no,
            },
        });
        if (customer) {
            const updateOtpResponse = await this.otpService.update(otp, customer.id);
            await this.customerService.updateoptpassword(Protected_password, customer.id, customer);
            const successResponse: any = {
                status: 1,
                message: 'Otp sent successfully please check your mobile',
                data: updateOtpResponse,
            };
            return response.status(200).send(successResponse);
        } else {
            const email = otpParam.mobile_no.concat("@gmail.com");
            const newCustomerParams = new Customer();
            newCustomerParams.username = otpParam.mobile_no;
            newCustomerParams.email = email;
            newCustomerParams.password = Protected_password;
            newCustomerParams.mobileNumber = otpParam.mobile_no;
            const customerSaveResponse = await this.customerService.create(newCustomerParams);
            const newOtpParams = new Otp();
            newOtpParams.mobile_no = otpParam.mobile_no;
            newOtpParams.otp = otp;
            newOtpParams.customer_id = customerSaveResponse.id;
            const otpSaveResponse = await this.otpService.create(newOtpParams);
            const successResponse: any = {
                status: 1,
                message: 'Otp sent successfully please check your mobile',
                data: otpSaveResponse,
            };
    ...
    // Create Verify Otp
    /**
     * @api {post} /api/auth/verify_otp Add Verify Otp API
     * @apiGroup Otp 
     * @apiParam (Request body) {String} mobile_no mobile_no
     ...
     *      "data" : {}
     * }
     * @apiSampleRequest /api/auth/verify_otp
     * @apiErrorExample {json} verifyOtp error
     * HTTP/1.1 500 Internal Server Error
     */
    @Post('/verify_otp')
    // @Authorized()
    public async verify(@Body({ validate: true }) VerifyOtpParam: VerifyOtpRequest, @Req() request: any, @Res() response: any): Promise<any> {
        const otp = await this.otpService.findOne({
            where: {
                mobile_no: VerifyOtpParam.mobile_no,
            },
        });
        const emailId = VerifyOtpParam.mobile_no.concat("@gmail.com");
        const otpstr = String(otp.otp);
        if (VerifyOtpParam.otp == otp.otp) {
            const data = {
                emailId: emailId,
                password: otpstr,
                type: 'normal',
            ...
            const verifyotpSaveResponse = await this.customerController.login(data, request, response);
            const successResponse: any = {
                data: verifyotpSaveResponse,
            };
            return response.status(200).send(successResponse);
        } else {
            const successOtpResponse: any = {
                message: 'Invalid otp',
            };
            return response.status(200).send(successOtpResponse);
