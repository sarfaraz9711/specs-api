...
export class CustomerCouponController {
    ...
    public async applyCoupon(@Req() request: any, @Res() response: any): Promise<any> {
        ...
        const checkCouponCode = await this.vendorCouponService.findOne({
            where: {
        ...
        const couponUsage = await this.couponUsageService.list(0, 0, [], [], whereConditionCoupon, true);
        ...
        const validCustomer = await this.customerService.findOne({ where: { email: emailId, deleteFlag: 0 } });
        if (validCustomer) {
            ...
            const customerUsagePerCoupon = await this.couponUsageService.list(0, 0, [], [], whereConditions, true);
        ...
        // }
        //Comment for use coupan for cart value 01-11-2022
        const error: any = [];
        let grandTotal = 0;
