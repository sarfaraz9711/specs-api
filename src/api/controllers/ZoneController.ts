...
export class ZoneController {
 ...
 // Zone All List API
    /**
     * @api {get} /api/zone/zone-all-list Zone All List API
     * @apiGroup Zone
     * @apiSuccessExample {json} Success
     * HTTP/1.1 200 OK
     * {
     ...
     *      }
     * }
     * @apiSampleRequest /api/zone/zone-all-list
     * @apiErrorExample {json} Zone error
     * HTTP/1.1 500 Internal Server Error
     */
    @Get('/zone-all-list')
    public async zoneAllList( @Res() response: any): Promise<any> {
        const zoneList = await this.zoneService.alllist();
        if (zoneList) {
            const successResponse: any = {
                status: 1,
                message: 'Successfully get all zone List',
