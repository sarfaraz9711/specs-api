    ...
    public async listByQueryBuilder(
        ...
        if (relations && relations.length > 0) {
            relations.forEach((joinTb: any) => {
                ...
                } else if (joinTb.op === 'joinop') {
                    query.innerJoin(`${joinTb.tableName}`, 'c','c.product_id=product.product_id');
                } else if (joinTb.op === 'joinC') {
                    query.innerJoin(`${joinTb.tableName}`, 'ct','ct.category_id=c.category_id');
                } else {
