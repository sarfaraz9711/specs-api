    ...
    public async listByQueryBuilder(
        ...
        if (relations && relations.length > 0) {
            relations.forEach((joinTb: any) => {
                if (joinTb.op === 'left-op') {
                    query.leftJoin(`${joinTb.tableName}`,'p','p.cn_applied_order_id=order.order_id');
                } else if (joinTb.op === 'left-pu') {
                    query.leftJoin(`${joinTb.tableName}`,'pu','pu.order_id=order.order_id');
        ...
        if (whereConditions && whereConditions.length > 0) {
            whereConditions.forEach((item: any) => {
                if (item.op === 'where' && item.sign === undefined) {
                    ...
                    let date_value=item.value.split(",")
                   query.andWhere(`Date(${item.name})` + ' BETWEEN ' + `Date('${date_value[0]}')` +' AND '+`Date('${date_value[1]}')`);
                }else if (item.op === 'like' && item.sign === undefined) {
                    query.andWhere(item.name + ' LIKE (' + item.value + ')');
                }else if (searchConditions.length==0 && item.op === 'GREATEREQUAL' && item.sign === undefined) {
                    query.andWhere(item.name + ' >=' + item.value );
                }
            });
        }
                ...
                if ((table.name && table.name instanceof Array && table.name.length > 0) && (table.value && table.value instanceof Array && table.value.length > 0)) {
                    ...
                    query.andWhere(new Brackets(qb => {
                        ...
                        namesArray.forEach((name: string, index: number) => {
                          ...
                          if (index === 0) {
                            qb.andWhere('LOWER(' + name + ')' + ' = ' + table.value );
                            return;
                        }
                        qb.orWhere('LOWER(' + name + ')' + ' = ' + table.value);
                    });
                }));
